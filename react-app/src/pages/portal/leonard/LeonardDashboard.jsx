...
const SEED_KPIS = [
  {
    label: 'Total Portfolio Value', value: '$2,847,350', change: '+12.3%', dir: 'up',
    sub: 'this quarter', icon: TrendingUp, bg: 'bg-blue-50', iconColor: 'text-blue-600',
  },
  {
    label: 'Active Clients', value: '14', change: '+2', dir: 'up',
    sub: 'this month', icon: Users, bg: 'bg-green-50', iconColor: 'text-green-600',
  },
  {
    label: 'Monthly Revenue', value: '$48,200', change: '+8.1%', dir: 'up',
    sub: 'vs last month', icon: DollarSign, bg: 'bg-emerald-50', iconColor: 'text-emerald-600',
  },
  {
    label: 'Properties', value: '7', change: 'Stable', dir: 'flat',
    sub: 'under management', icon: Building2, bg: 'bg-amber-50', iconColor: 'text-amber-600',
  },
  {
    label: 'Digital Assets', value: '$847,000', change: '+22.1%', dir: 'up',
    sub: 'this quarter', icon: BarChart3, bg: 'bg-purple-50', iconColor: 'text-purple-600',
  },
  {
    label: 'Crypto Holdings', value: '$312,500', change: '-4.2%', dir: 'down',
    sub: '7-day change', icon: Bitcoin, bg: 'bg-orange-50', iconColor: 'text-orange-600',
  },
  {
    label: 'Open Incidents', value: '2', change: '-1', dir: 'good_down',
    sub: 'resolved today', icon: ShieldAlert, bg: 'bg-red-50', iconColor: 'text-red-500',
  },
  {
    label: 'Threat Level', value: 'Low', change: 'All Clear', dir: 'safe',
    sub: 'security status', icon: Shield, bg: 'bg-green-50', iconColor: 'text-green-600',
  },
];
...
function KpiChangeIndicator({ dir, change }) {
  if (dir === 'up')        return <><TrendingUp size={11} className="text-green-500" /><span className="text-xs font-medium text-green-600">{change}</span></>;
  if (dir === 'down')      return <><AlertTriangle size={11} className="text-red-400" /><span className="text-xs font-medium text-red-500">{change}</span></>;
  if (dir === 'good_down') return <><TrendingUp size={11} className="text-green-500 rotate-180" /><span className="text-xs font-medium text-green-600">{change}</span></>;
  if (dir === 'safe')      return <><CheckCircle size={11} className="text-green-500" /><span className="text-xs font-medium text-green-600">{change}</span></>;
  return <span className="text-xs font-medium text-gray-400">{change}</span>;
}
    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <div className="flex items-center gap-1.5 text-xs text-gray-400 mb-1">
            <span className="text-gray-500 font-medium">Leonard Portal</span>
          ...
          return (
            <div key={kpi.label} className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              ...
              <div className="flex items-center gap-1 mt-1 flex-wrap">
                <KpiChangeIndicator dir={kpi.dir} change={kpi.change} />
      ...
      <div className="grid lg:grid-cols-3 gap-4">
              ...
              <div key={item.label}>
                <div className="flex justify-between text-xs text-gray-600 mb-1">
                  ...
                  <span className="text-gray-500">
                    {item.amount} <span className="text-gray-400">({item.pct}%)</span>
                  </span>
                </div>
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
          ...
          <div className="space-y-0 divide-y divide-gray-50 max-h-[340px] overflow-y-auto pr-1">
            {SEED_ACTIVITY.map((item, i) => (
              <div key={i} className="flex items-center gap-3 py-2.5">
                <div
                  className={`h-7 w-7 rounded-full flex items-center justify-center flex-shrink-0 ${
                    ACTIVITY_TYPE_COLOR[item.type] || 'bg-gray-100 text-gray-500'
                  }`}
                >
                  <Activity size={12} />
                </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Q2 2026</span>
        </div>
